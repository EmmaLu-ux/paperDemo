...
from tools.print_format import print_success
...
def message_deal(message):
    ...
    if message['m_type'] == 'tx_broadcast':
        ...
        if not MESSAGES[m_hash].commit_state and len(MESSAGES[m_hash].commit_m) >= VERIFY_LIMIT:
            ...
            print_success('-----MESSAGES------:' + str(len(MESSAGES)))

            if len(MESSAGES) == (DEMO_AMOUNT*4):
                end = int(round(time.time() * 1000))
                time_ = file2json(COST_TIME_PATH)
                time_[TEST_KEY]['end'] = end

                cost_time = (time_[TEST_KEY]['end'] - time_[TEST_KEY]['start'])
                time_[TEST_KEY]['average'] = cost_time / (DEMO_AMOUNT*4)
                print_success('-----cost time(ms)------:'+ str(cost_time))
                print_success('-----average time(ms)------:'+ str(cost_time/(DEMO_AMOUNT*4)))
                json2file(COST_TIME_PATH, time_)
                input('@')
