...
from config.config import MESSAGE_QUEUE, MESSAGE_TH_EVENT
...
class Node(threading.Thread):
    ...
    def connect_with_node(self, host, port, reconnect=False):
        ...
        try:
            ...
            # sock.send('-1') # Send my id to the connected node!
            ...
            print("connecting to %s port %s successfully" % (host, port))
    ...
    def node_message(self, node, data):
        """This method is invoked when a node send us a message.
           ...
           s_block_top_hash                    send node top block hash
           s_block_raw_data                    send node top block hash
           r_block_top_hash                    request node top block hash
           r_block_raw_data                    request block raw_data
           spread_tx                           spread tx
           new_block                           spread new block
        """
        print("node_message: " + str(data))

        MESSAGE_QUEUE.put(data)
        MESSAGE_TH_EVENT.set()
